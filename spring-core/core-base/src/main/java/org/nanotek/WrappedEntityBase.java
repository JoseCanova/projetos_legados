...
public interface WrappedEntityBase<K extends BaseBean<?>> extends Wrapper <K>{
	...
	static <K extends WrappedBaseClass<?> , I extends BaseBean<?>> I newBaseBeanInstance(Class<I> beanClass) throws BaseInstantiationException { 
		try {
				return BaseBean.newBaseBeanInstance(beanClass).get();
		} catch (Exception e) {
	...
	static <K extends WrappedBaseClass<?>> Optional<?> newInstance(Class<?> clazz , Object[] args , Class<?>... classArgs  ) throws BaseInstantiationException { 
		try {
